...
class CartController {
    constructor() {
        ...
        this.updateCartItem = (0, withTransaction_1.default)(async (req, res, next, tx) => {
            var _a, _b, _c, _d;
            // Zschema validation can be added here if needed
            console.log("Request Body: ", req.body);
            const payload = cart_Zschema_1.cartSchema.parse(req.body);
            ...
            // এখন payload safe
            console.log("Validated Payload: ", payload);
            const cartServiceResult = await cart_service_1.default.updateCartItem(payload, tx);
            const resDoc = (0, responseHandler_1.responseHandler)(200, "Cart updated", cartServiceResult);
            res.status(resDoc.statusCode).json(resDoc);
        });
        this.cartProductConfirm = (0, withTransaction_1.default)(async (req, res, next, tx) => {
            var _a, _b, _c, _d;
            // Zschema validation can be added here if needed
            console.log("Request Body: ", req.body);
            ...
            console.log("Validated Payload: ", payload);
            const cartServiceResult = await cart_service_1.default.cartProductConfirm(payload, tx);
            const resDoc = (0, responseHandler_1.responseHandler)(200, "Cart product confirmed", cartServiceResult);
            res.status(resDoc.statusCode).json(resDoc);
        });
