...
class CountryZoneController {
  createCountryZone = withTransaction(
    async (req: Request, res: Response, next: NextFunction, tx: any) => {
      try {
        ...
        const resDoc = responseHandler(
          201,
          "CountryZone Created successfully",
          countryZone,
        );
  ...
  getCountryZoneForShipping = async (
    req: Request,
    res: Response,
    next: NextFunction,
  ) => {
    try {
      ...
      const resDoc = responseHandler(
        200,
        "CountryZone for shipping retrieved successfully",
        countryZone,
      );
      res.status(resDoc.statusCode).json(resDoc);
    } catch (error) {
  ...
  exportCountryZoneData = async (
    ...
    try {
      ...
      const resDoc = responseHandler(
        200,
        "CountryZone data exported successfully",
        exportResult,
      );
      res.status(resDoc.statusCode).json(resDoc);
    } catch (error) {
  ...
  getCountryZoneWithPagination = withTransaction(
    async (req: Request, res: Response, next: NextFunction, tx: any) => {
      const page = parseInt(req.query.page as string, 10) || 1;
      const limit = parseInt(req.query.limit as string, 10) || 10;
      const payload = { page, limit };
      const countries = await countryZoneService.getCountryZoneWithPagination(
        payload,
        tx,
      );
      const resDoc = responseHandler(
  ...
  updateCountryZone = withTransaction(
    async (req: Request, res: Response, next: NextFunction, tx: any) => {
      try {
        ...
        const updatedCountryZone = await countryZoneService.updateCountryZone(
          id,
          payload,
          tx,
        );
        const resDoc = responseHandler(
          200,
          `CountryZone with id ${id} updated successfully`,
          updatedCountryZone,
        );
        res.status(resDoc.statusCode).json(resDoc);
      } catch (error) {
  ...
  deleteCountryZone = async (
    ...
    try {
      ...
      const resDoc = responseHandler(
        200,
        `CountryZone with id ${id} deleted successfully`,
        null,
      );
      res.status(resDoc.statusCode).json(resDoc);
    } catch (error) {
