...
class SeaBookingController {
  createSeaBooking = withTransaction(
    async (req: Request, res: Response, next: NextFunction, tx: any) => {
      ...
      const seaBookingResult = await SeaBookingService.createSeaBooking(
        payload,
        payloadFiles,
        tx,
      );
      const resDoc = responseHandler(
        201,
        "SeaBooking Created successfully",
        seaBookingResult,
      );
  ...
  getAllSeaBookingByFilterWithPagination = catchError(
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      const seaBookingResult =
        await SeaBookingService.getAllSeaBookingByFilterWithPagination(payload);
      const resDoc = responseHandler(
        200,
        "Get All SeaBookings",
        seaBookingResult,
  ...
  updateSeaBooking = catchError(
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      const seaBookingResult = await SeaBookingService.updateSeaBooking(
        id,
        payload,
        payloadFiles,
      );
