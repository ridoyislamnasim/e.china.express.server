...
class WarehouseController {
  createWarehouse = withTransaction(
  ...
  getWarehousesWithPagination = withTransaction(
    async (req: Request, res: Response, next: NextFunction, tx: any) => {
      try {
        ...
        const warehouses = await warehouseService.getWarehousesWithPagination(
          filter,
          tx,
        );
        const resDoc = responseHandler(
          200,
          "Warehouses retrieved successfully with pagination",
          warehouses,
        );
        res.status(resDoc.statusCode).json(resDoc);
      } catch (error) {
  ...
  updateWarehouse = withTransaction(
    async (req: Request, res: Response, next: NextFunction, tx: any) => {
      try {
        const { id } = req.params;
        const payload = req.body;
  ...
  updateWarehouseCapacity = withTransaction(
    async (req: Request, res: Response, next: NextFunction, tx: any) => {
      try {
        const { id } = req.params;
  ...
  getAvailableWarehouses = async (
    ...
    try {
      const minAvailableCapacity = req.query.minAvailableCapacity
        ...
        : 0;
      const warehouses =
        await warehouseService.getAvailableCapacityWarehouses(
  ...
  changeWarehouseStatus = withTransaction(
    async (req: Request, res: Response, next: NextFunction, tx: any) => {
      try {
        const { id } = req.params;
