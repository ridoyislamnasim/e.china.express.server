...
class SubCategoryController {
  createSubCategory = withTransaction(
  ...
  getSubCategoryWithPagination = catchError(
    async (req: Request, res: Response, next: NextFunction) => {
      let payload = {
        page: req.query.page,
      ...
      const subCategory =
        await SubCategoryService.getSubCategoryWithPagination(payload);
      const resDoc = responseHandler(
        200,
  ...
  getSingleSubCategory = catchError(
    async (req: Request, res: Response, next: NextFunction) => {
      const slug = req.params.slug as string;
      const subCategoryResult =
        await SubCategoryService.getSingleSubCategory(slug);
      const resDoc = responseHandler(
        201,
