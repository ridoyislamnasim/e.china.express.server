...
export class CartService extends BaseService<typeof cartRepository> {
  ...
  const productTotalWeight = cartProductAllVariants[0].variants.reduce((sum: number, cp: any) => sum + (cp.weight ?? 0) * (cp.quantity ?? 0), 0);
    ...
    const payloadInfo = {
      countryCombinationId: cartProductAllVariants[0].productShipping[0].rate?.countryCombinationId, 
    ...
    const updateCartProductShippingRate = await this.repository.updateCartProductShipping(
      cartProductAllVariants[0].productShipping[0].id,
