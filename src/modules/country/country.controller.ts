...
class CountryController {
  ...
  getCountryForShipping = async (
    ...
    try {
      ...
      const resDoc = responseHandler(
        200,
        "Country for shipping retrieved successfully",
        country,
      );
      res.status(resDoc.statusCode).json(resDoc);
    } catch (error) {
  ...
  exportCountryData = async (
    ...
    try {
      ...
      const resDoc = responseHandler(
        200,
        "Country data exported successfully",
        exportResult,
      );
      res.status(resDoc.statusCode).json(resDoc);
    } catch (error) {
  ...
  getCountryWithPagination = withTransaction(
    async (req: Request, res: Response, next: NextFunction, tx: any) => {
      const page = parseInt(req.query.page as string, 10) || 1;
      const limit = parseInt(req.query.limit as string, 10) || 10;
      const payload = { page, limit };
      const countries = await countryService.getCountryWithPagination(
        payload,
        tx,
      );
      const resDoc = responseHandler(
  ...
  updateCountry = withTransaction(
    async (req: Request, res: Response, next: NextFunction, tx: any) => {
      try {
        ...
        const resDoc = responseHandler(
          200,
          `Country with id ${id} updated successfully`,
          updatedCountry,
        );
        res.status(resDoc.statusCode).json(resDoc);
      } catch (error) {
