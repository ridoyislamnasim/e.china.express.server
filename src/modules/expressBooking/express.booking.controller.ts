...
class ExpressBookingController {
  createExpressBooking = withTransaction(
    async (req: Request, res: Response, next: NextFunction, tx: any) => {
      ...
      const expressBookingResult =
        await ExpressBookingService.createExpressBooking(
          payload,
          payloadFiles,
          tx,
        );
      const resDoc = responseHandler(
        201,
        "ExpressBooking Created successfully",
        expressBookingResult,
      );
      res.status(resDoc.statusCode).json(resDoc);
    },
  );

  getAllExpressBookingByFilterWithPagination = catchError(
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      const expressBookingResult =
        await ExpressBookingService.getAllExpressBookingByFilterWithPagination(
          payload,
        );
      const resDoc = responseHandler(
        200,
  ...
  getSingleExpressBooking = catchError(
    async (req: Request, res: Response, next: NextFunction) => {
      const id = req.params.id as string;
      const expressBookingResult =
        await ExpressBookingService.getSingleExpressBooking(id);
      const resDoc = responseHandler(
        201,
        "Single ExpressBooking successfully",
        expressBookingResult,
      );
      res.status(resDoc.statusCode).json(resDoc);
    },
  ...
  updateExpressBooking = catchError(
    async (req: Request, res: Response, next: NextFunction) => {
      const id = req.params.id as string;
      const payloadFiles = {
        files: req.files,
      ...
      const expressBookingResult =
        await ExpressBookingService.updateExpressBooking(
          id,
          payload,
          payloadFiles,
        );
  ...
  deleteExpressBooking = catchError(
    async (req: Request, res: Response, next: NextFunction) => {
      const id = req.params.id as string;
      const expressBookingResult =
        await ExpressBookingService.deleteExpressBooking(id);
      const resDoc = responseHandler(
        200,
        "ExpressBooking Deleted successfully",
      );
      res.status(resDoc.statusCode).json(resDoc);
    },
  );
