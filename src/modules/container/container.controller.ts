...
class ContainerController {
  createContainer = withTransaction(
  ...
  getContainerWithPagination = withTransaction(
    async (req: Request, res: Response, next: NextFunction, tx: any) => {
      const page = parseInt(req.query.page as string, 10) || 1;
      const limit = parseInt(req.query.limit as string, 10) || 10;
      const payload = { page, limit };
      const countries = await containerService.getContainerWithPagination(
        payload,
        tx,
      );
      const resDoc = responseHandler(
  ...
  updateContainer = withTransaction(
    async (req: Request, res: Response, next: NextFunction, tx: any) => {
      try {
        ...
        const resDoc = responseHandler(
          200,
          `Container with id ${id} updated successfully`,
          updatedContainer,
        );
        res.status(resDoc.statusCode).json(resDoc);
      } catch (error) {
  ...
  deleteContainer = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const resDoc = responseHandler(
        200,
        `Container with id ${id} deleted successfully`,
        null,
      );
      res.status(resDoc.statusCode).json(resDoc);
    } catch (error) {
