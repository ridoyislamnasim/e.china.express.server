...
export default new (class ServicesController {
  ...
  createServiceType = catchError(async (req: Request, res: Response) => {
    ...
    const newServiceType = await servicesService.createServiceType(
      payload as CreateServiceTypeRequestDTO,
    );
  ...
  getAllServiceTypes = catchError(async (req: Request, res: Response) => {
    ...
    const resDoc = responseHandler(
      201,
      "All service types fetched successfully.",
      allServiceTypes,
    );
    res.status(resDoc.statusCode).json(resDoc);
  });

  getServiceTypesWithPagination = withTransaction(
    async (req: Request, res: Response, next: NextFunction) => {
      const page = Math.max(1, Number(req.query.page) || 1);
      const limit = Math.min(100, Math.max(1, Number(req.query.limit) || 10));
      console.log("🚀 ~ services.controller.ts:114 ~ limit:", limit);

      const data = await servicesService.getServiceTypesWithPagination({
        page,
        limit,
      });
      ...
      const resDoc = responseHandler(
        200,
        "Service types retrieved successfully with pagination.",
        data,
      );
      res.status(resDoc.statusCode).json(resDoc);
    },
  ...
  updateService = catchError(async (req: Request, res: Response) => {
    const slug = req.params.slug as string;
    const { title, description, serviceTypeId, serviceTypeTitle, id } =
      req.body;
    const payload = {
      title,
      description,
  ...
  getServicesWithPagination = withTransaction(
    async (req: Request, res: Response, next: NextFunction, tx: any) => {
      ...
      const resDoc = responseHandler(
        200,
        "Services retrieved successfully with pagination.",
        data,
      );
      res.status(resDoc.statusCode).json(resDoc);
    },
  ...
  deleteService = catchError(async (req: Request, res: Response) => {
    ...
    const resDoc = responseHandler(
      200,
      `Service with slug ${slug} deleted successfully.`,
      null,
    );
    res.status(resDoc.statusCode).json(resDoc);
  });
  ...
  addHelpfulCount = catchError(async (req: Request, res: Response) => {
    console.log(
      "🚀 ~ services.controller.ts:202 ~ req:",
      req.params.slug,
      req.query.increase,
    );
    const slug = req.params.slug as string;
