...
function restoreDB(connectionUrl: string) {
    ...
    const command =
        `${pgRestorePath} -h ${config.host} -U ${config.user} -d ${config.database} -v -c ${QUOTED_BACKUP_PATH}`;

    console.log("🔄 Restoring Database...");
    exec(command, { env: envVars }, (error: ExecException | null, stdout: string, stderr: string) => {
        if (error) {
